...
if [ "$1" != "" ]; then
    ...
    su zabbix -s "/bin/bash" -c "/usr/sbin/zabbix_proxy --foreground -c /etc/zabbix/zabbix_proxy.conf"
elif [ "$zbx_type" == "server" ]; then
    echo "** Starting Zabbix server"
    su zabbix -s "/bin/bash" -c "/usr/sbin/zabbix_server --foreground -c /etc/zabbix/zabbix_server.conf"
elif [ "$zbx_type" == "java-gateway" ]; then
    echo "** Starting Zabbix Java Gateway"
    su zabbix -s "/bin/bash" -c "/usr/sbin/zabbix_java_gateway"
elif [ "$zbx_type" == "frontend" ] && [ "$zbx_opt_type" == "apache" ]; then
    echo "** Starting Zabbix frontend"
    if [ -f "/usr/sbin/httpd" ]; then
        /usr/sbin/httpd -D FOREGROUND
    elif [ -f "/usr/sbin/apache2ctl" ]; then
        /bin/bash -c "source /etc/apache2/envvars && /usr/sbin/apache2ctl -D FOREGROUND"
    else
